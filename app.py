...
Run:  uv run streamlit run app.py
      (or)  streamlit run app.py  inside the activated venv
"""
...
with st.sidebar:
    ...
    # Computed base payments preview
    def _pi(loan, rate, n):
        mr = rate / 100 / 12
...
@st.cache_data
def get_schedule(hp, dp, rate, term, extra, lump, lump_mo, tax, ins, hoa_v,
                 rent_v, vac, mgmt, maint):
    lp = LoanParams(hp, dp, rate / 100, term, extra, lump, lump_mo)
    cp = CostParams(tax, ins, hoa_v)
    rp = RentalParams(rent_v, vac, mgmt, maint)
    return build_schedule(lp, cp, rp)
...
def equity_chart(rows15, rows30):
    """Plotly chart: equity % over time, both loans."""
    fig = go.Figure()
    # 15-yr
    x15 = [r.month for r in rows15]
    y15 = [r.equity_pct for r in rows15]
    fig.add_trace(go.Scatter(x=x15, y=y15, name="15-yr", mode="lines",
                             line=dict(color="#38bdf8", width=2)))
    # 30-yr
    x30 = [r.month for r in rows30]
    y30 = [r.equity_pct for r in rows30]
    fig.add_trace(go.Scatter(x=x30, y=y30, name="30-yr", mode="lines",
...
def cashflow_chart(rows):
    """Plotly chart: monthly net CF over time."""
    months = [r.month for r in rows]
    ncf    = [r.net_cash_flow for r in rows]
    colors = ["#4ade80" if v >= 0 else "#f87171" for v in ncf]
    ...
    fig.add_hline(y=0, line_color="#6b7fa8", line_width=1)
    fig.update_layout(
        title="Monthly Net Cash Flow (Rental)",
        xaxis_title="Month",
        yaxis_title="$ / month",
        plot_bgcolor="#0f1117",
        paper_bgcolor="#0f1117",
...
def interest_breakdown_chart(stats15, stats30):
    """Plotly grouped bar: principal vs interest for both loans."""
    fig = go.Figure(data=[
        go.Bar(name="15-yr Principal", x=["15-Year", "30-Year"],
               y=[loan_amount, loan_amount],
               marker_color="#1e4a6e"),
        go.Bar(name="15-yr Interest",  x=["15-Year", "30-Year"],
...
with tab15:
    ...
    with c1:
        st.plotly_chart(cashflow_chart(rows15), use_container_width=True)
    with c2:
        # Milestone callouts
        st.markdown("**Equity Milestones**")
        for pct, mo in mil15.items():
            yr = 2026 + (mo - 1) // 12
            st.success(f"**{pct}% equity** — Month {mo} ({yr})")
...
with tab30:
    ...
    c1, c2 = st.columns([3, 2])
    with c1:
        st.plotly_chart(cashflow_chart(rows30), use_container_width=True)
    with c2:
        st.markdown("**Equity Milestones**")
        for pct, mo in mil30.items():
            yr = 2026 + (mo - 1) // 12
            st.info(f"**{pct}% equity** — Month {mo} ({yr})")
